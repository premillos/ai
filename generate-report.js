...
const META_HISTORY_FILE = path.join(OUTPUT_DIRECTORY, 'meta-history.html');
...
const META_HISTORY_DATA_FILE = path.join(DATA_DIRECTORY, 'meta-history.json');
...
/**
 * 规范搜索结果地址，移除常见跟踪参数后用于跨日期识别同一页面。
 * @param {string} link 搜索结果网址
 * @returns {string} 规范网址
 */
function normalizeResultUrl(link) {
  try {
...
function buildRankTrendData(reports) {
  ...
  const buildTrend = (keywordFilter, countryFilter) => {
    ...
    const getDomainProfileStats = (domain) => {
      if (!domainProfileStats.has(domain)) {
        domainProfileStats.set(domain, {
          appearances: 0,
          top3Appearances: 0,
          top10Appearances: 0,
          keywords: new Map(),
      ...
      return domainProfileStats.get(domain);
    };

    // 累计关键词或国家维度的出现次数和排名，用于生成网站优势画像。
    const appendProfileDimension = (dimension, name, position) => {
      const current = dimension.get(name) ?? { appearances: 0, rankTotal: 0 };
      current.appearances += 1;
      current.rankTotal += position;
      dimension.set(name, current);
    };
      ...
      for (const keyword of visibleKeywords) {
        ...
        for (const query of visibleQueries) {
          for (const result of query.results) {
            ...
            const profileStats = getDomainProfileStats(result.domain);
            profileStats.appearances += 1;
            if (result.position <= 3) profileStats.top3Appearances += 1;
            if (result.position <= 10) profileStats.top10Appearances += 1;
            appendProfileDimension(
              profileStats.keywords,
              keyword.name,
              result.position,
            );
            appendProfileDimension(
              profileStats.countries,
              query.country,
              result.position,
            );
    ...
    const buildProfileDimension = (dimension) =>
      [...dimension.entries()]
        .map(([name, item]) => ({
          name,
          appearances: item.appearances,
          averageRank:
            Math.round((item.rankTotal / item.appearances) * 10) / 10,
        }))
        .sort(
          (left, right) =>
    ...
    const series = domains.map((domain) => {
      ...
      const observedPoints = points.filter((point) => point.rank !== null);
      const profileStats = domainProfileStats.get(domain);
      return {
        domain,
        points,
        profile: {
          appearances: profileStats.appearances,
          keywordCount: profileStats.keywords.size,
          countryCount: profileStats.countries.size,
          top3Rate:
            Math.round(
              (profileStats.top3Appearances / profileStats.appearances) * 1000,
            ) / 10,
          top10Rate:
            Math.round(
              (profileStats.top10Appearances / profileStats.appearances) * 1000,
            ) / 10,
          firstSeen: observedPoints[0]?.date ?? null,
...
/**
 * 按相同关键词、国家和网址生成 Title 与 Description 历史。
 * @param {object[]} reports 按日期生成的完整报告
 * @returns {object} 搜索展示文案历史数据
 */
function buildMetaHistoryData(reports) {
  const historyMap = new Map();
  const dates = [...new Set(reports.map((report) => report.snapshotDate))].sort();
  ...
  for (const report of [...reports].sort((left, right) =>
    ...
    for (const keyword of report.keywords) {
      for (const query of keyword.queries) {
        for (const result of query.results) {
          const url = normalizeResultUrl(result.link);
          if (!url) continue;
          const key = JSON.stringify([keyword.name, query.country, url]);
          ...
          const snapshot = {
            ...
            description: normalizeMetadataText(result.snippet),
            position: result.position,
          };
          const current = history.snapshotsByDate.get(report.snapshotDate);
          if (!current || snapshot.position < current.position) {
            history.snapshotsByDate.set(report.snapshotDate, snapshot);
          }
        }
      }
    }
  ...
  const histories = [...historyMap.values()].map((history) => {
    let previous = null;
    let titleChangeCount = 0;
    let descriptionChangeCount = 0;
    const snapshots = [...history.snapshotsByDate.values()]
      .sort((left, right) => left.date.localeCompare(right.date))
      .map((snapshot) => {
        const titleChanged = previous !== null && snapshot.title !== previous.title;
        const descriptionChanged =
          previous !== null && snapshot.description !== previous.description;
        if (titleChanged) titleChangeCount += 1;
        if (descriptionChanged) descriptionChangeCount += 1;
        previous = snapshot;
        return { ...snapshot, titleChanged, descriptionChanged };
      });
    return {
      ...
      firstSeen: snapshots[0]?.date ?? null,
      lastSeen: snapshots.at(-1)?.date ?? null,
      titleChangeCount,
  ...
  histories.sort(
    (left, right) =>
      right.titleChangeCount +
        right.descriptionChangeCount -
        (left.titleChangeCount + left.descriptionChangeCount) ||
  ...
  return {
    dates,
    keywords: [...new Set(histories.map((history) => history.keyword))].sort(),
    countries: [...new Set(histories.map((history) => history.country))].sort(),
    domains: [...new Set(histories.map((history) => history.domain))].sort(),
    histories,
  };
}
...
${DATA_LOADER_STYLES}
    ...
    .analysis-metric::after {
      ...
      height: 90px;
      border-radius: 50%;
      background: var(--metric-glow, rgba(42, 157, 143, 0.1));
      content: '';
    }
    ...
    .change-value.up { color: #14866d; }
    .change-value.down { color: #d95e41; }
    .change-value.new { color: #2774a7; }
    .change-value.lost { color: #8b6bb1; }
    ...
    @media (max-width: 700px) {
      ...
      /* Flatpickr 默认宽度为 330px，320px 设备会越出视口。 */
      ...
      .flatpickr-calendar .flatpickr-day { max-width: none; }
      .legend-toolbar { display: block; }
      .legend-actions { margin-top: 12px; }
      .analysis-heading,
      .profile-heading { display: block; }
      .analysis-range,
      .profile-select { display: block; margin-top: 10px; }
...
<body>
  <main class="page">
    ...
    <section class="trend-section" id="trends" aria-labelledby="trend-title">
      ...
      <div class="legend-toolbar" aria-label="网站批量筛选">
        <span class="legend-selection-summary" id="legend-selection-summary" aria-live="polite">正在准备网站选项</span>
        <div class="legend-actions" role="group" aria-label="批量选择网站">
          <button class="legend-action" type="button" data-legend-action="all">全选</button>
          <button class="legend-action" type="button" data-legend-action="invert">反选</button>
          <button class="legend-action" type="button" data-legend-action="default">默认前 10</button>
          <button class="legend-action" type="button" data-legend-action="none">清空</button>
        </div>
      </div>
      <p class="trend-note" id="trend-note"></p>
    </section>

    <section class="analysis-section" id="analysis" aria-labelledby="analysis-title">
      <div class="analysis-heading">
        <div>
          <h2 id="analysis-title">排名变化分析</h2>
          <p>基于当前关键词、国家、日期和网站选择，观察变化、波动与竞争画像。</p>
        </div>
        <span class="analysis-range" id="analysis-range">正在计算真实数据</span>
      </div>
      <div class="analysis-metrics" id="analysis-metrics" aria-live="polite"></div>
      <div class="analysis-grid">
        <article class="analysis-panel">
          ...
          <div class="analysis-list" id="ranking-changes"></div>
        </article>
        <article class="analysis-panel">
          <div class="panel-heading">
            <div>
              <h3>波动指数</h3>
              <p>0–100 分，分数越高代表排名越不稳定</p>
            </div>
          </div>
      ...
      <article class="profile-panel">
        <div class="profile-heading">
          <div>
            <h3>网站画像</h3>
            <p>排名表现、覆盖能力与优势市场的综合视图</p>
          </div>
          <select class="profile-select" id="profile-domain-select" aria-label="选择画像网站"></select>
        </div>
        <div id="domain-profile" aria-live="polite"></div>
      </article>
    </section>
  ...
  <script>
    ...
    const profileDomainSelections = new Map();
    ...
    // 计算网站在当前日期范围内的排名变化与波动指标。
    function calculateSeriesAnalytics(series, visibleDates, maximumRank) {
      const ranksByDate = new Map(
        series.points.map((point) => [point.date, point.rank])
      );
      const ranks = visibleDates.map((date) => ranksByDate.get(date) ?? null);
      const observedRanks = ranks.filter((rank) => rank !== null);
      const startRank = ranks[0] ?? null;
      const endRank = ranks.at(-1) ?? null;
      const averageRank = observedRanks.length === 0
        ? null
        : Math.round(
            (observedRanks.reduce((total, rank) => total + rank, 0) /
              observedRanks.length) * 10
          ) / 10;
      let volatilityTotal = 0;
      let volatilityTransitions = 0;
      const missingRank = maximumRank + 1;

      for (let index = 1; index < ranks.length; index += 1) {
        const previous = ranks[index - 1];
        const current = ranks[index];
        if (previous === null && current === null) continue;
        volatilityTotal += Math.abs(
          (previous ?? missingRank) - (current ?? missingRank)
        );
        volatilityTransitions += 1;
      }

      const volatility = volatilityTransitions === 0
        ? null
        : Math.min(
            100,
            Math.round(
              (volatilityTotal / volatilityTransitions / maximumRank) * 1000
            ) / 10
          );
      ...
      if (visibleDates.length < 2) {
        ...
        change = Math.round((startRank - endRank) * 10) / 10;
      ...
      return {
        ...
        endRank,
        averageRank,
        bestRank: observedRanks.length ? Math.min(...observedRanks) : null,
        worstRank: observedRanks.length ? Math.max(...observedRanks) : null,
        observedCount: observedRanks.length,
        coverageRate: visibleDates.length === 0
          ? 0
          : Math.round((observedRanks.length / visibleDates.length) * 1000) / 10,
        volatility,
        change,
        changeType
      };
    ...
    function getVolatilityLevel(score) {
      ...
      if (score <= 15) return '稳定';
      if (score <= 35) return '轻微波动';
    ...
    function buildActiveAnalytics(trend, visibleDates, selectedState) {
      ...
      const averageVolatility = volatilityItems.length === 0
        ? null
        : Math.round(
            (volatilityItems.reduce((total, item) => total + item.volatility, 0) /
              volatilityItems.length) * 10
          ) / 10;
      return {
        items,
        averageVolatility,
        upCount: items.filter((item) => item.changeType === 'up').length,
        downCount: items.filter((item) => item.changeType === 'down').length,
        newCount: items.filter((item) => item.changeType === 'new').length,
        lostCount: items.filter((item) => item.changeType === 'lost').length
      };
    }

    // 创建分析概览指标卡。
    function renderAnalysisMetrics(analytics) {
      const container = document.getElementById('analysis-metrics');
      const metrics = [
        {
          label: '监测网站',
          value: analytics.items.length,
          detail: '当前筛选范围内有排名数据',
          glow: 'rgba(42, 157, 143, 0.12)'
        },
        {
          label: '平均波动指数',
          value: analytics.averageVolatility ?? '--',
          detail: getVolatilityLevel(analytics.averageVolatility),
          glow: 'rgba(233, 162, 59, 0.13)'
        },
        {
          label: '排名上升',
          value: analytics.upCount + analytics.newCount,
          detail: analytics.newCount + ' 个新进入榜单',
          glow: 'rgba(39, 116, 167, 0.12)'
        },
        {
          label: '排名下降',
          value: analytics.downCount + analytics.lostCount,
          detail: analytics.lostCount + ' 个掉出榜单',
          glow: 'rgba(238, 108, 77, 0.12)'
        }
      ];
      container.replaceChildren();
      metrics.forEach((metric) => {
        const card = createElement('article', 'analysis-metric');
        card.style.setProperty('--metric-glow', metric.glow);
        card.append(
    ...
    // 选择网站画像并将视线移动到画像区域。
    function selectProfileDomain(trend, domain, shouldScroll = false) {
      profileDomainSelections.set(trend.key, domain);
      const selectedState = getLegendSelection(trend);
      renderAnalytics(trend, getModernVisibleDates(trend), selectedState);
      if (shouldScroll) {
    ...
    function formatRankChange(item) {
      if (item.changeType === 'up') return '上升 ' + item.change + ' 位';
      if (item.changeType === 'down') return '下降 ' + Math.abs(item.change) + ' 位';
      if (item.changeType === 'new') return '新进入';
      if (item.changeType === 'lost') return '掉出榜单';
      return '排名持平';
    }

    // 渲染排名变化榜，点击网站可直接查看画像。
    function renderRankingChanges(trend, analytics) {
      const container = document.getElementById('ranking-changes');
      const changePriority = { new: 4, lost: 3, up: 2, down: 2, stable: 1 };
      const items = [...analytics.items]
        .filter((item) => item.changeType !== 'insufficient')
        .sort((left, right) => {
          const leftMagnitude = Math.abs(left.change ?? trend.maximumRank);
          const rightMagnitude = Math.abs(right.change ?? trend.maximumRank);
          return (
            rightMagnitude - leftMagnitude ||
            changePriority[right.changeType] - changePriority[left.changeType] ||
            left.domain.localeCompare(right.domain)
          );
        })
        .slice(0, 8);
      ...
      if (items.length === 0) {
        container.appendChild(
          createElement('div', 'analysis-empty', '当前日期范围不足以计算排名变化')
        );
        return;
      }
      items.forEach((item) => {
        ...
        button.type = 'button';
        const detail = item.startRank === null
          ? '未入榜 → 第 ' + item.endRank + ' 名'
          : item.endRank === null
          ? '第 ' + item.startRank + ' 名 → 未入榜'
          : '第 ' + item.startRank + ' 名 → 第 ' + item.endRank + ' 名';
        ...
        const value = createElement(
          'span',
          'change-value ' + item.changeType,
          formatRankChange(item)
        );
        button.append(content, value);
        button.addEventListener('click', () =>
    ...
    // 渲染波动指数排行榜。
    function renderVolatilityRanking(trend, analytics) {
      const container = document.getElementById('volatility-ranking');
      const items = [...analytics.items]
        .filter((item) => item.volatility !== null)
        .sort(
          (left, right) =>
            right.volatility - left.volatility ||
            left.domain.localeCompare(right.domain)
        )
        .slice(0, 8);
      container.replaceChildren();
      if (items.length === 0) {
        container.appendChild(
          createElement('div', 'analysis-empty', '至少需要两个真实数据日才能计算波动指数')
        );
        return;
      }
      items.forEach((item) => {
        ...
        content.append(
          createElement('strong', '', item.domain),
          createElement('small', '', getVolatilityLevel(item.volatility))
        );
        const score = createElement('span');
        score.append(
          createElement('span', 'volatility-value', item.volatility),
          createElement('span', 'volatility-track')
        );
        score.lastChild.appendChild(createElement('span', 'volatility-fill'));
        score.lastChild.lastChild.style.width = item.volatility + '%';
    ...
    // 渲染网站优势关键词和国家标签。
    function createProfileStrength(title, items, formatter) {
      const panel = createElement('section', 'profile-strength');
      panel.appendChild(createElement('h4', '', title));
      const tags = createElement('div', 'profile-tags');
      if (items.length === 0) {
        tags.appendChild(createElement('span', 'profile-tag', '暂无数据'));
      } else {
        items.forEach((item) => {
          tags.appendChild(createElement('span', 'profile-tag', formatter(item)));
        });
      }
    ...
    // 渲染选中网站的排名、覆盖能力和雷达画像。
    function renderDomainProfile(trend, analytics) {
      const container = document.getElementById('domain-profile');
      const select = document.getElementById('profile-domain-select');
      const domains = analytics.items.map((item) => item.domain);
      let activeDomain = profileDomainSelections.get(trend.key);
      if (!domains.includes(activeDomain)) activeDomain = domains[0] ?? null;
      profileDomainSelections.set(trend.key, activeDomain);
      select.replaceChildren();
      domains.forEach((domain) => {
        const option = createElement('option', '', domain);
        option.value = domain;
      ...
      container.replaceChildren();
      if (profileRadar) {
        profileRadar.dispose();
        profileRadar = null;
      }
      const item = analytics.items.find((entry) => entry.domain === activeDomain);
      if (!item) {
        container.appendChild(
          createElement('div', 'analysis-empty', '请至少选择一个有排名数据的网站')
        );
        return;
      }

      const layout = createElement('div', 'profile-layout');
      const radarElement = createElement('div', 'profile-radar');
      radarElement.setAttribute('role', 'img');
      radarElement.setAttribute('aria-label', item.domain + ' 网站画像雷达图');
      const details = createElement('div');
      details.appendChild(createElement('h4', 'profile-domain', item.domain));
      details.appendChild(
        createElement(
          'p',
          'profile-meta',
          '首次出现 ' + (item.profile.firstSeen ?? '--') +
            ' · 最近出现 ' + (item.profile.lastSeen ?? '--')
        )
      );
      const stats = createElement('div', 'profile-stats');
      stats.append(
        createProfileStat('当前排名', item.endRank === null ? '未入榜' : '第 ' + item.endRank + ' 名'),
        createProfileStat('区间平均', item.averageRank === null ? '--' : '第 ' + item.averageRank + ' 名'),
      ...
      const strengths = createElement('div', 'profile-strengths');
      strengths.append(
        createProfileStrength(
          '优势关键词',
          item.profile.topKeywords,
          (entry) => entry.name + ' · 平均第 ' + entry.averageRank + ' 名'
        ),
        createProfileStrength(
          '优势国家',
          item.profile.topCountries,
          (entry) => formatCountry(entry.name) + ' · 平均第 ' + entry.averageRank + ' 名'
        )
      );
      details.appendChild(strengths);
      layout.append(radarElement, details);
      container.appendChild(layout);

      if (window.echarts) {
        const rankingStrength = item.averageRank === null
          ? 0
          : Math.max(
              0,
              Math.round(
                (1 -
                  (item.averageRank - 1) /
                    Math.max(1, trend.maximumRank - 1)) *
                  100
              )
            );
        profileRadar = window.echarts.init(radarElement);
        profileRadar.setOption({
          ...
          radar: {
            ...
            axisName: { color: '#5d687c', fontSize: 10 },
            splitArea: {
              areaStyle: { color: ['#fbfcfe', '#f6f9fb'] }
            },
            splitLine: { lineStyle: { color: '#dfe6ee' } },
            axisLine: { lineStyle: { color: '#dfe6ee' } }
          },
          series: [{
            type: 'radar',
            symbolSize: 5,
            areaStyle: { color: 'rgba(42, 157, 143, 0.18)' },
            lineStyle: { width: 2 },
            data: [{
              name: item.domain,
              value: [
                rankingStrength,
                item.volatility === null ? 0 : 100 - item.volatility,
                item.coverageRate,
                item.profile.top3Rate,
                item.profile.top10Rate
              ]
            }]
          }]
        });
    ...
    // 同步当前筛选条件下的全部分析模块。
    function renderAnalytics(trend, visibleDates, selectedState) {
      const analytics = buildActiveAnalytics(trend, visibleDates, selectedState);
      const range = document.getElementById('analysis-range');
      range.textContent = visibleDates.length === 0
        ? '当前范围无数据'
    ...
    // 批量更新当前筛选组合的网站图例，减少逐个点击操作。
    function setModernLegendSelection(mode) {
      const trend = getActiveTrend();
      if (!trend) return;
      const currentSelection = getLegendSelection(trend);
      const nextSelection = Object.fromEntries(
        trend.series.map((item, index) => {
          let selected = false;
          if (mode === 'all') selected = true;
          if (mode === 'invert') selected = !Boolean(currentSelection[item.domain]);
          if (mode === 'default') selected = index < defaultVisibleDomainCount;
          return [item.domain, selected];
        })
      );
      legendSelections.set(trend.key, nextSelection);
      renderModernTrend();
    }

    // 同步批量操作状态、已选数量和趋势辅助说明。
    function updateLegendControls(trend, selectedState, visibleDates) {
      const selectedCount = trend.series.reduce(
        (count, item) => count + (selectedState[item.domain] ? 1 : 0),
        0
      );
      const summary = document.getElementById('legend-selection-summary');
      summary.textContent = '已选择 ' + selectedCount + ' / ' + trend.series.length + ' 个网站';
      document.querySelectorAll('[data-legend-action]').forEach((button) => {
        button.disabled =
          (button.dataset.legendAction === 'all' && selectedCount === trend.series.length) ||
          (button.dataset.legendAction === 'none' && selectedCount === 0);
      });
    ...
    function renderModernTrend() {
      ...
      updateLegendControls(trend, selectedState, visibleDates);
      renderAnalytics(trend, visibleDates, selectedState);
    }
    ...
    document.querySelectorAll('[data-legend-action]').forEach((button) => {
      button.addEventListener('click', () => {
        setModernLegendSelection(button.dataset.legendAction);
      });
    });

    document.getElementById('profile-domain-select').addEventListener('change', (event) => {
      const trend = getActiveTrend();
      if (trend) selectProfileDomain(trend, event.target.value);
    });
    ...
    if (rankingChart) {
      rankingChart.on('legendselectchanged', (event) => {
        const trend = getActiveTrend();
        if (trend) {
          legendSelections.set(trend.key, event.selected);
          updateLegendControls(
            trend,
            event.selected,
            getModernVisibleDates(trend)
          );
          renderAnalytics(
            trend,
            getModernVisibleDates(trend),
      ...
      rankingChart.on('click', (event) => {
        const trend = getActiveTrend();
        if (trend && event.componentType === 'series' && event.seriesName) {
          selectProfileDomain(trend, event.seriesName, true);
        }
      });
      window.addEventListener('resize', () => {
        rankingChart.resize();
        if (profileRadar) profileRadar.resize();
      });
    }
...
/**
 * 生成搜索结果 Title 与 Description 历史页面。
 * @param {string} dataVersion 文案历史数据内容版本号
 * @returns {string} HTML 内容
 */
function renderMetaHistoryHtml(dataVersion) {
  const pageTitle = `Title 与 Description 历史 · ${SITE_NAME}`;
  const pageDescription =
    '按关键词、国家和页面网址追踪搜索结果中的 Title 与 Description 历史变化，定位文案调整时间。';
  const structuredData = {
    '@context': 'https://schema.org',
    '@type': 'CollectionPage',
    '@id': `${SITE_URL}/meta-history.html#webpage`,
    url: `${SITE_URL}/meta-history.html`,
    name: pageTitle,
    description: pageDescription,
    inLanguage: 'zh-CN',
    isPartOf: { '@id': `${SITE_URL}/#website` },
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    body {
      ...
      color: var(--ink);
      background:
        radial-gradient(circle at 8% 0%, rgba(8, 145, 178, 0.11), transparent 30rem),
        radial-gradient(circle at 92% 12%, rgba(15, 159, 143, 0.09), transparent 26rem),
        var(--paper);
    ...
    .eyebrow { margin: 0 0 12px; color: var(--cyan); font-size: 11px; font-weight: 760; letter-spacing: 0.16em; text-transform: uppercase; }
    h1 { max-width: 900px; margin: 0; font-size: clamp(38px, 5vw, 76px); line-height: 0.98; letter-spacing: -0.055em; }
    .hero p:last-child { max-width: 760px; margin: 22px 0 0; color: var(--soft); line-height: 1.8; }
    .scope-note {
      padding: 20px; border: 1px solid rgba(8, 145, 178, 0.18); border-radius: 18px;
      background: rgba(255, 255, 255, 0.64); color: var(--soft); font-size: 13px; line-height: 1.7;
      backdrop-filter: blur(14px);
    }
    .scope-note strong { display: block; margin-bottom: 5px; color: var(--ink); font-size: 14px; }
    .metrics { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 12px; margin-bottom: 18px; }
    .metric { min-height: 104px; padding: 18px; border: 1px solid var(--line); border-radius: 17px; background: var(--surface); }
    .metric span { color: var(--soft); font-size: 12px; }
    .metric strong { display: block; margin-top: 9px; font-size: 28px; font-variant-numeric: tabular-nums; }
    .filters {
      position: sticky; top: 10px; z-index: 5; display: grid;
      grid-template-columns: minmax(220px, 1.25fr) repeat(4, minmax(140px, 0.65fr));
      gap: 10px; padding: 13px; border: 1px solid rgba(220, 229, 239, 0.9);
      border-radius: 18px; background: rgba(255, 255, 255, 0.86);
      box-shadow: 0 16px 42px rgba(15, 23, 42, 0.09); backdrop-filter: blur(18px);
    }
    .control { display: grid; gap: 6px; }
    .control label { color: var(--soft); font-size: 10px; letter-spacing: 0.1em; text-transform: uppercase; }
    input, select { width: 100%; height: 40px; padding: 0 12px; border: 1px solid var(--line); border-radius: 11px; background: #fff; color: var(--ink); outline: none; }
    input:focus, select:focus { border-color: var(--cyan); box-shadow: 0 0 0 3px rgba(8, 145, 178, 0.1); }
    .result-bar { display: flex; justify-content: space-between; gap: 20px; padding: 25px 4px 13px; color: var(--soft); font-size: 13px; }
    .result-bar strong { color: var(--ink); }
    .history-list { display: grid; gap: 12px; }
    .history-card { overflow: hidden; border: 1px solid var(--line); border-radius: 18px; background: var(--surface); box-shadow: 0 8px 28px rgba(15, 23, 42, 0.045); }
    .history-card[open] { border-color: rgba(8, 145, 178, 0.28); box-shadow: 0 18px 44px rgba(15, 23, 42, 0.08); }
    summary { display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: 18px; padding: 18px 20px; cursor: pointer; list-style: none; }
    summary::-webkit-details-marker { display: none; }
    .summary-main { min-width: 0; }
    .domain-row { display: flex; align-items: center; gap: 10px; margin-bottom: 8px; }
    .domain-row strong { font-size: 15px; }
    .condition { color: var(--soft); font-size: 11px; }
    .latest-title { overflow: hidden; margin: 0; font-size: 16px; line-height: 1.45; text-overflow: ellipsis; white-space: nowrap; }
    .latest-description { display: -webkit-box; overflow: hidden; margin: 7px 0 0; color: var(--soft); font-size: 12px; line-height: 1.6; -webkit-box-orient: vertical; -webkit-line-clamp: 2; }
    .change-badges { display: flex; align-items: center; justify-content: flex-end; gap: 7px; min-width: 190px; }
    .badge { padding: 6px 9px; border-radius: 999px; font-size: 11px; white-space: nowrap; }
    .badge.title { background: rgba(8, 145, 178, 0.1); color: #087a98; }
    .badge.description { background: rgba(234, 106, 71, 0.1); color: #c64e30; }
    .badge.stable { background: rgba(100, 116, 139, 0.09); color: var(--soft); }
    .timeline { display: grid; gap: 0; padding: 0 20px 20px; border-top: 1px solid #edf2f7; }
    .snapshot { position: relative; display: grid; grid-template-columns: 112px minmax(0, 1fr); gap: 16px; padding: 18px 0 4px; }
    .snapshot + .snapshot { border-top: 1px dashed #e5ebf2; }
    .snapshot-meta { color: var(--soft); font-size: 11px; line-height: 1.8; }
    .snapshot-meta time { display: block; color: var(--ink); font-size: 12px; font-weight: 700; }
    .field { padding: 10px 12px; border-left: 3px solid transparent; border-radius: 0 10px 10px 0; background: rgba(248, 250, 252, 0.8); }
    .field + .field { margin-top: 7px; }
    .field.changed { border-left-color: var(--orange); background: rgba(255, 247, 244, 0.88); }
    .field-label { display: flex; align-items: center; gap: 8px; margin-bottom: 4px; color: var(--soft); font-size: 10px; font-weight: 760; letter-spacing: 0.08em; text-transform: uppercase; }
    .field-label em { padding: 2px 5px; border-radius: 5px; background: rgba(234, 106, 71, 0.12); color: var(--orange); font-style: normal; letter-spacing: 0; }
    .field p { margin: 0; font-size: 13px; line-height: 1.65; overflow-wrap: anywhere; }
    .page-link { display: inline-block; overflow: hidden; max-width: 100%; margin-top: 12px; color: var(--cyan); font-size: 11px; text-overflow: ellipsis; white-space: nowrap; text-decoration: none; }
    .load-more { display: none; width: 100%; margin-top: 16px; padding: 13px; border: 1px solid var(--line); border-radius: 13px; background: #fff; color: var(--ink); cursor: pointer; }
    .load-more:hover { border-color: var(--cyan); color: var(--cyan); }
    .empty { padding: 80px 20px; border: 1px dashed var(--line); border-radius: 18px; color: var(--soft); text-align: center; }
    .loading { padding: 90px 20px; color: var(--soft); text-align: center; }
    .footer { padding-top: 42px; color: var(--soft); font-size: 12px; text-align: center; }
    @media (max-width: 1050px) {
    ...
    @media (max-width: 720px) {
      ...
      .change-badges { justify-content: flex-start; min-width: 0; }
      .snapshot { grid-template-columns: 1fr; gap: 8px; }
    }
  </style>
</head>
<body>
  <main class="page">
    ...
    <section class="hero">
      <div>
        <p class="eyebrow">SERP Metadata Timeline</p>
        <h1>Title 与 Description 历史</h1>
        <p>追踪页面在搜索结果中的标题和描述变化，快速确认哪一天发生调整、哪些关键词和国家受到影响。</p>
      </div>
      <aside class="scope-note">
        <strong>对比口径</strong>
        仅比较相同关键词、相同国家和相同页面 URL 的相邻可见记录。这里的 Description 来自搜索结果摘要，不等同于网页源代码中的 Meta Description。
      </aside>
    </section>

    <section class="metrics" id="meta-metrics" aria-label="文案变化概览"></section>
    <section class="filters" aria-label="文案历史筛选">
      <div class="control">
        <label for="meta-search">搜索页面或文案</label>
        <input id="meta-search" type="search" placeholder="输入域名、URL、Title 或 Description">
      </div>
      <div class="control">
        <label for="meta-status">变化类型</label>
        <select id="meta-status">
          <option value="changed" selected>全部变化</option>
          <option value="title">Title 变化</option>
          <option value="description">Description 变化</option>
          <option value="both">两者均变化</option>
          <option value="stable">暂无变化</option>
          <option value="all">全部记录</option>
        </select>
      </div>
      <div class="control"><label for="meta-keyword">关键词</label><select id="meta-keyword"></select></div>
      <div class="control"><label for="meta-country">国家</label><select id="meta-country"></select></div>
      <div class="control"><label for="meta-domain">网站</label><select id="meta-domain"></select></div>
    </section>

    <div class="result-bar"><span id="meta-result-count">正在读取数据</span><span id="meta-date-range"></span></div>
    <section class="history-list" id="meta-history-list" aria-live="polite"><div class="loading">正在整理 Title 与 Description 时间线…</div></section>
    <button class="load-more" id="meta-load-more" type="button">加载更多</button>
    <footer class="footer">前端AI实验室 · Copyright © pzl</footer>
  </main>

  <script>
    (async () => {
      const response = await fetch('data/meta-history.json?v=${encodeURIComponent(dataVersion)}');
      if (!response.ok) throw new Error('文案历史数据请求失败：HTTP ' + response.status);
      const data = await response.json();
      const list = document.getElementById('meta-history-list');
      const searchInput = document.getElementById('meta-search');
      const statusSelect = document.getElementById('meta-status');
      const keywordSelect = document.getElementById('meta-keyword');
      const countrySelect = document.getElementById('meta-country');
      const domainSelect = document.getElementById('meta-domain');
      const loadMoreButton = document.getElementById('meta-load-more');
      const countryNames = { in: '印度', us: '美国', gb: '英国', ca: '加拿大', br: '巴西', fr: '法国', ph: '菲律宾', mx: '墨西哥' };
      const pageSize = 30;
      ...
      function appendOptions(select, values, formatter) {
        select.replaceChildren();
        values.forEach((value) => {
          const option = createElement('option', '', formatter(value));
          option.value = value;
          select.appendChild(option);
        });
      }
      appendOptions(keywordSelect, ['all', ...data.keywords], (value) => value === 'all' ? 'All' : value);
      appendOptions(countrySelect, ['all', ...data.countries], (value) => value === 'all' ? 'All' : countryNames[value] ?? value.toUpperCase());
      appendOptions(domainSelect, ['all', ...data.domains], (value) => value === 'all' ? 'All' : value);

      function matchesStatus(item, status) {
        const titleChanged = item.titleChangeCount > 0;
        const descriptionChanged = item.descriptionChangeCount > 0;
        if (status === 'changed') return titleChanged || descriptionChanged;
        if (status === 'title') return titleChanged;
        if (status === 'description') return descriptionChanged;
        if (status === 'both') return titleChanged && descriptionChanged;
        if (status === 'stable') return !titleChanged && !descriptionChanged;
        return true;
      }

      function applyFilters() {
        const query = searchInput.value.trim().toLocaleLowerCase();
        filteredItems = data.histories.filter((item) => {
          const latest = item.snapshots.at(-1);
          const searchable = [item.domain, item.url, latest?.title, latest?.description]
            .join(' ')
            .toLocaleLowerCase();
          return matchesStatus(item, statusSelect.value) &&
            (keywordSelect.value === 'all' || item.keyword === keywordSelect.value) &&
            (countrySelect.value === 'all' || item.country === countrySelect.value) &&
            (domainSelect.value === 'all' || item.domain === domainSelect.value) &&
      ...
      function renderSnapshot(snapshot) {
        const row = createElement('article', 'snapshot');
        const meta = createElement('div', 'snapshot-meta');
        const date = createElement('time', '', snapshot.date);
        date.dateTime = snapshot.date;
        meta.append(date, createElement('span', '', '自然排名：第 ' + snapshot.position + ' 名'));
        const fields = createElement('div');
        [
          { label: 'Title', value: snapshot.title || '暂无标题', changed: snapshot.titleChanged },
          { label: 'Description', value: snapshot.description || '暂无描述', changed: snapshot.descriptionChanged }
        ].forEach((item) => {
          const field = createElement('div', 'field' + (item.changed ? ' changed' : ''));
          const label = createElement('div', 'field-label', item.label);
          if (item.changed) label.appendChild(createElement('em', '', '本次变化'));
          field.append(label, createElement('p', '', item.value));
      ...
      function renderCard(item, index) {
        const latest = item.snapshots.at(-1);
        const card = createElement('details', 'history-card');
        if (index < 2) card.open = true;
        const header = document.createElement('summary');
        const main = createElement('div', 'summary-main');
        const domainRow = createElement('div', 'domain-row');
        domainRow.append(
          createElement('strong', '', item.domain),
          createElement('span', 'condition', item.keyword + ' · ' + (countryNames[item.country] ?? item.country.toUpperCase()))
        );
        main.append(
          domainRow,
          createElement('p', 'latest-title', latest?.title || '暂无标题'),
          createElement('p', 'latest-description', latest?.description || '暂无描述')
        );
        const badges = createElement('div', 'change-badges');
        if (item.titleChangeCount > 0) badges.appendChild(createBadge('title', 'Title ' + item.titleChangeCount + ' 次'));
        if (item.descriptionChangeCount > 0) badges.appendChild(createBadge('description', 'Description ' + item.descriptionChangeCount + ' 次'));
        if (item.titleChangeCount === 0 && item.descriptionChangeCount === 0) badges.appendChild(createBadge('stable', '暂无变化'));
        header.append(main, badges);
        const timeline = createElement('div', 'timeline');
        [...item.snapshots].reverse().forEach((snapshot) => timeline.appendChild(renderSnapshot(snapshot)));
        const link = createElement('a', 'page-link', item.url + ' ↗');
        link.href = item.url;
        link.target = '_blank';
      ...
      function renderMetrics() {
        const changedItems = data.histories.filter((item) => item.titleChangeCount > 0 || item.descriptionChangeCount > 0);
        const metrics = [
          ['监测页面条件', data.histories.length],
          ['Title 变化', data.histories.reduce((total, item) => total + item.titleChangeCount, 0)],
          ['Description 变化', data.histories.reduce((total, item) => total + item.descriptionChangeCount, 0)],
          ['受影响网站', new Set(changedItems.map((item) => item.domain)).size]
        ];
        const container = document.getElementById('meta-metrics');
        metrics.forEach(([label, value]) => {
          const card = createElement('article', 'metric');
      ...
      function render() {
        ...
        if (visibleItems.length === 0) {
          list.appendChild(createElement('div', 'empty', '当前筛选条件下没有匹配的文案历史'));
        } else {
          visibleItems.forEach((item, index) => list.appendChild(renderCard(item, index)));
        }
        const resultCount = document.getElementById('meta-result-count');
        resultCount.replaceChildren(
          '找到 ',
          createElement('strong', '', filteredItems.length),
          ' 条页面条件历史'
        );
        loadMoreButton.style.display = visibleCount < filteredItems.length ? 'block' : 'none';
      }

      [statusSelect, keywordSelect, countrySelect, domainSelect].forEach((select) =>
      ...
      document.getElementById('meta-date-range').textContent =
        data.dates.length === 0 ? '暂无数据日期' : data.dates[0] + ' → ' + data.dates.at(-1);
      renderMetrics();
      applyFilters();
    })().catch((error) => {
      const list = document.getElementById('meta-history-list');
      list.replaceChildren();
      list.appendChild(document.createTextNode('文案历史加载失败：' + error.message));
      console.error(error);
    });
  </script>
</body>
...
function renderSitemap(reports) {
  ...
  const staticUrls = [
    ...
    {
      location: `${SITE_URL}/meta-history.html`,
      lastModified: latestDate,
      changeFrequency: 'daily',
      priority: '0.8',
    },
...
async function main() {
  ...
  await Promise.all([
    ...
    writeFile(
      META_HISTORY_FILE,
      renderMetaHistoryHtml(metaHistoryVersion),
      'utf8',
    ),
    ...
    // 文案历史独立加载，避免大量搜索展示文本进入 HTML。
    writeFile(
      META_HISTORY_DATA_FILE,
      `${metaHistoryJson}\n`,
      'utf8',
    ),
  ...
  console.log(
    `报告生成完成：${INDEX_FILE}、${ARCHIVE_FILE}、${RANK_ANIMATION_FILE}、${META_HISTORY_FILE}、${SITEMAP_FILE}（${reportSummaries.length} 个历史数据页）`,
  );
}
